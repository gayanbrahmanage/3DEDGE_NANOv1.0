...
LRFB_widget::LRFB_widget(int w, int h){
  set_size_request(w,h);
  update(0.0,0.0,0.0,0.0);
}

bool LRFB_widget::on_draw(const Cairo::RefPtr<Cairo::Context>& cr){
  ...
  cr->set_line_width(1.0);
  cr->rectangle(0.1*h/2, h/2-rw/2, rl, rw);
  cr->fill();
  cr->stroke();
...
void LRFB_widget::update(double l, double r, double f,double b){
  posL = l;
  posR = r;
  posF = f;
  posB = b;
